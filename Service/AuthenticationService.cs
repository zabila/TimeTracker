...
public sealed class AuthenticationService : IAuthenticationService {
    ...
    public async Task<bool> ValidateUser(UserForAuthenticationDto userForAuth) {
        _user = await _userManager.FindByNameAsync(userForAuth.UserName ?? throw new InvalidOperationException("User name is null"));
        Debug.Assert(_user != null, nameof(_user) + " != null");
        var result = await _userManager.CheckPasswordAsync(_user, userForAuth.Password ?? throw new InvalidOperationException("Password is null"));
        if(!result)
    ...
    private Task<SigningCredentials> GetSigningCredentials() {
        var key = Encoding.UTF8.GetBytes(_configuration["SECRET"] ?? throw new InvalidOperationException("SECRET is null"));
